...
if($SelfTest){
  ...
  if($Headers.Count -ne 80){throw "CSV schema invariant failed: $($Headers.Count) columns."}
  @{ok=$true;schema='ACLM native telemetry 1.1';columns=$Headers.Count;rate_hz=$RateHz;wear_precision='IEEE-754 single round-trip';aid_tire_rate=$true}|ConvertTo-Json -Compress
  exit 0
}
