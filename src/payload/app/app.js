...
; Generated by ACLM Historical Tire Lab v0.5.5
...
`;
 ...
 files["tyres.ini"]=ini;
 files["ACLM_TIREPACK_MANIFEST.txt"]=`ACLM Historical Tire Lab v0.5.5
Car: ${v("car")}
...
const ACLM_APP_VERSION="0.4.5";let availableUpdate=null;
...
async function checkForUpdates(silent=false){const status=$("updateStatus"),btn=$("installUpdate");if(!silent)status.textContent="Checking ACLM Tire Tool Updates…";btn.disabled=true;availableUpdate=null;try{const r=await fetch("/api/update-info?ts="+Date.now(),{cache:"no-store"});const data=await r.json();if(!r.ok||data.error)throw new Error(data.error||("HTTP "+r.status));if(isNewerVersion(data.version,ACLM_APP_VERSION)){availableUpdate=data;btn.disabled=false;status.innerHTML=`<span class="warning"><b>Update available: v${escapeHtml(data.version)}</b></span> — installed v${ACLM_APP_VERSION}${data.notes?`<br>${escapeHtml(data.notes)}`:""}`;}else{status.innerHTML=`<span class="ok"><b>Up to date.</b></span> Installed v${ACLM_APP_VERSION}; stable channel v${escapeHtml(data.version||ACLM_APP_VERSION)}.`;}}catch(e){status.innerHTML=`<span class="warning"><b>Update check unavailable:</b></span> ${escapeHtml(e.message)}`;}}
async function installAvailableUpdate(){
 if(!availableUpdate)return;
 const url=availableUpdate.release_page||availableUpdate.updates_folder;
 if(!url){$("updateStatus").innerHTML='<span class="warning"><b>No release page was provided.</b></span>';return;}
...
async function syncKnowledge(force=false){
 const st=$("knowledgeStatus");st.textContent=force?"Synchronizing latest ACLM tire knowledge…":"Checking ACLM tire knowledge…";
 try{
   ...
   window.ACLMHistoricalCategories.loadKnowledgeRelease(data.release,data.source||"Google Drive release");
   updateKnowledgeUi(`<span class="ok"><b>Knowledge synchronized:</b></span> v${escapeHtml(data.release.releaseVersion||"?")} · schema ${escapeHtml(data.release.schemaVersion||"?")} · ${data.updated?"new cache installed":"already current"}.`);
 }catch(e){st.innerHTML=`<span class="warning"><b>Knowledge sync unavailable:</b></span> ${escapeHtml(e.message)} Last-known-good knowledge remains active.`;}
}
$("checkKnowledge").addEventListener("click",()=>syncKnowledge(false));
$("syncKnowledge").addEventListener("click",()=>syncKnowledge(true));
setTimeout(async()=>{await loadCurrentKnowledge();syncKnowledge(false);},250);
